    ...
    <nav className="flex items-center justify-between mt-3 mb-12">
      ...
      <div className="flex items-center justify-end gap-4 ml-6">
        ...
        <Link href="/category/mobile-accessories">
          <h5 className="hover:underline">Accessories</h5>
        </Link>
      </div>
      <div className="flex items-center">
        ...
        <Link href="/pay" className="relative">
          ...
          {cart.length > 0 && <span className="absolute -top-2 -right-2 bg-red-500 text-white text-[10px] font-bold rounded-full h-5 w-5 flex items-center justify-center">{cart.length}</span>}
        </Link>
      </div>
    </nav>
  );
